...
module.exports = app => {
    ...
    router.post("/usuario", validatorSchema(usuarioSchema), usuario.create); //http://localhost:9595/api/usuario
    router.put("/usuarios/:id", authRequired, isAdmin, usuario.update); //http://localhost:9595/api/usuarios/1

    router.get("/usuarios/", usuario.findAll); //http://localhost:9595/api/usuarios/
    router.get("/usuarios/:id",  usuario.findOne); //http://localhost:9595/api/usuarios/1
